 ...
 * 
 * @author Alexis Devillard and Tiphaine Diot Attributes : serialVersionUID,
 *         graphicInterface, myEnv, myBots[], time, GUI, ma_fenetre,
 *         buttonPanel, onMapPanel, onMapButtons[], onMapItem[] behaviorPanel,
 *         behaviorBUttons[], behaviorType[], GUIactive Functions :
 *         getters/setters, addOnMap(), addBots(), start(), update(), initGUI(),
 *         display(), displaySensors(), actionPerformed
 */
public class Simulation extends JPanel implements ActionListener {
	...
	/**
	 * Environment of the simulation: containing the dim of the room and the
	 * different obstacles and durts
	 */
	 ...
	 * @param gui
	 *            True for a graphical mode, false for a terminal mode
	 */
	...
	public void addOnMap(int i) {
		...
		case 1:// obstacle
			this.myEnv
					.addObst(new Obstacle((int) (this.myEnv.getWidth() * Math
							.random()), (int) (this.myEnv.getHeigth() * Math
							.random())));
			break;
		case 2:// dirt
			this.myEnv
					.addDurt(new Durt((int) (this.myEnv.getWidth() * Math
							.random()), (int) (this.myEnv.getHeigth() * Math
							.random())));
			break;
		}
	...
	public void update() {
		...
		for (int i = 0; i < this.myBots.size(); i++)
			System.out.print("\t#robot n°" + i + ": "
					+ this.myBots.get(i).generateNext(this.myEnv));
	...
	public void initGUI() {
		...
		setOpaque(true);
		if (this.myEnv != null)
			this.setPreferredSize(new Dimension(this.myEnv.getWidth(),
					this.myEnv.getHeigth())); // Taille de la fenetre
		else
			this.setPreferredSize(new Dimension(200, 200)); // Taille de la
															// fenetre
		...
		// On crée une première grille pour les boutons ajoutant des objets de
		// la carte
		onMapPanel.setLayout(new GridLayout(1, 3));// GridLayout(int rows, int
													// cols)
		// la fonction setBorder nous permet de séparer proprement les
		// différents panels que nous utilisons
		onMapPanel.setBorder(BorderFactory.createEmptyBorder(6, 2, 2, 3));// createEmptyBorder(North
																			// ,
																			// West
																			// ,
																			// South
		...
		for (int i = 0; i < 3; i++) {
			this.onMapButtons[i] = new JButton(this.onMapItem[i]);// on crée le
																	// bouton
			this.onMapPanel.add(this.onMapButtons[i]);// on ajoute le bouton au
														// panel
			this.onMapButtons[i].addActionListener(this);// Pour chaque bouton
															// on ajoute une
															// action d'écoute

			this.behaviorButtons[i] = new JButton(this.behaviorType[i]);// on
																		// crée
																		// le
																		// bouton
			this.behaviorPanel.add(this.behaviorButtons[i]);// on ajoute le
															// bouton au panel
			this.behaviorButtons[i].addActionListener(this);// Pour chaque
															// bouton on ajoute
															// une action
															// d'écoute
		}
		...
		// on y ajoute les deux panel précedant et on insert des label
		// Expplicatifs
		buttonPanel.add(new JLabel("OBJECTS", JLabel.CENTER));
		buttonPanel.add(new JLabel("(Click to randomly put one on the map)",
				JLabel.CENTER));
		buttonPanel.add(onMapPanel, BorderLayout.WEST); // Le contenu est
														// l'objet Move
		buttonPanel.add(new JLabel("BEHAVIORS", JLabel.CENTER));
		buttonPanel.add(new JLabel("(Click to set robot(s) behavior)",
				JLabel.CENTER));
	...
	public void paint(Graphics g) {
		...
		g2.fillRect(0, 0, (int) g2.getClipBounds().getWidth(), (int) g2
	...
	public void display(OnMap obj, Graphics g) {
		...
		// Ici le code est modulable, il suffit de rajouter une forme et de
		// rajouter un elseif ATTENTION il faut modifier le code à d'autres
		// endroits également

		if (obj.getShape() == OnMap.shapes[0])// circle
			g2.fillOval((int) pos.getX() - size / 2, (int) pos.getY() - size
					/ 2, size, size);
		else if (obj.getShape() == OnMap.shapes[1])// square
			g2.fillRect((int) pos.getX(), (int) pos.getY(),
					(int) obj.getSize(), (int) obj.getSize());
	...
	public void displaySensor(Robot rob, Graphics g) {
		...
		for (int i = 0; i < rob.nbObstSensor(); i++) {
			...
			double ptx = rob.getPos().getX() + dx * Math.cos(thetar) + dy
					* Math.sin(thetar);
			double pty = rob.getPos().getY() - dx * Math.sin(thetar) + dy
					* Math.cos(thetar);
